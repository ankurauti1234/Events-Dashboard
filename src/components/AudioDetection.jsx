...
import { Download, AlertCircle, Music2 } from "lucide-react";
...
export function AudioDetectionTable({
  data,
  convertTimestamp,
  onExport,
  onPageChange,
      ...
      <Card className={compact ? "w-full p-2" : "w-full"}>
        <CardHeader className={compact ? "p-2" : ""}>
          ...
          <CardDescription className={compact ? "text-sm" : ""}>
            View and manage audio detection events
          </CardDescription>
        </CardHeader>
        <CardContent className={compact ? "p-2" : ""}>
          ...
          >
            <div className={`rounded-full bg-muted ${compact ? "p-4" : "p-6"}`}>
              <Music2 className={compact ? "h-8 w-8" : "h-12 w-12"} />
            </div>
            <div className="text-center space-y-2">
              ...
              >
                There are currently no audio detection events available for this
      ...
      <CardHeader className={compact ? "space-y-2 p-2" : "space-y-4"}>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription className={compact ? "text-sm" : ""}>
              Showing {data.events.length} of {data.total} total events
            </CardDescription>
          </div>
          <TooltipProvider>
            <Tooltip>
...
export default AudioDetectionTable;
